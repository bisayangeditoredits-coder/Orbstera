/**
 * Legacy export worker: HTTP callback to /api/export/pptx (serverless).
 * Use only when EXPORT_WORKER_INLINE=false.
 */
...
async function loop() {
  ...
  for (;;) {
    ...
    console.log('[export-worker:legacy] processing', jobId);
    try {
      ...
      console.log('[export-worker:legacy] completed', jobId);
    } catch (e) {
      console.error('[export-worker:legacy] failed', jobId, e);
      await redis.set(
        `job:v1:${jobId}`,
        {
          id: jobId,
